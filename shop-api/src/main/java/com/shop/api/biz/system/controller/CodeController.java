...
import com.shop.core.enums.ExcelTitleCode;
import com.shop.core.enums.LangType;
import com.shop.core.utils.CommUtil;
import com.shop.core.utils.FileUtil;
...
public class CodeController {
    ...
    public ApiResponse<PageResponse<CodeResponse.Paging>> selectCodePaging(
            @Parameter(name = "CodeRequestPagingFilter", description = "코드관리 목록 조회 (페이징) 필터", in = ParameterIn.QUERY) CodeRequest.PagingFilter filter,
    ...
    public ApiResponse<List<CodeResponse.CodeDropDown>> selectDropdownByCodeUpper(
        ...
        */
        ...
        if (StringUtils.equals("10020", codeRequest.getCodeUpper())) {
     ...
     * @return
     */
    @GetMapping(value = "/funcCd/{codeUpper}")
    @Operation(summary = "하위 코드 조회")
    public ApiResponse<List<CodeResponse.CodeDropDown>> getFuncCdListByCodeUpper(
            @Parameter(description = "상위_코드") @PathVariable String codeUpper
    ) {
        // 필수값 체크
        if (StringUtils.isEmpty(codeUpper.toString())) {
            return new ApiResponse<>(ApiResultCode.NO_REQUIRED_VALUE);
        }

        // 하위_코드_조회
        List<CodeResponse.CodeDropDown> codeList = codeService.getFuncCdListByCodeUpper(codeUpper);
    ...
    public ApiResponse updateCode(
        ...
        // 필수값 체크
        if(StringUtils.isBlank(codeRequest.getDelYn())){
        ...
        // 세션정보를 이용해서 수정자 세팅
        codeRequest.setUpdUser(jwtUser.getLoginId());
     ...
     */
    @AccessLog("코드 수정")
    @PutMapping("/updateCodeUseNotUse")
    @Operation(summary = "코드 수정")
    public ApiResponse updateCodeUseNotUse(
            @Parameter(hidden = true) @JwtUser User jwtUser,
            @Parameter(description = "코드 수정 Request") @RequestBody CodeRequest.Update codeRequest)
    {
        CodeResponse.Select codeResponse = codeService.selectCodeById(codeRequest.getId());
