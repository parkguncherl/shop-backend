...
public class PageResponse<T> {
    ...
    // 페이징 처리후 focused되어야할 row 를 지정해준다.
    public PageResponse(int page, int pageRowCount, int focusedIndex, List<T> list, long totalRowCount) {
        this.page = page;
        this.pageRowCount = pageRowCount;
        this.pageBlockSize = 10;
        this.rows = list;
    ...
    private long getStartRowNumer() {
        if(this.hasContents()) {
            if(this.totalRowCount < this.pageRowCount) {
                ...
                return this.totalRowCount - ((long) (getPage() - 1) * this.pageRowCount);
            }
        } else {
            return 0L;
        }
    ...
    public PageObject getPaging() {
        ...
        pageObject.setFocusedIndex(this.focusedIndex);
